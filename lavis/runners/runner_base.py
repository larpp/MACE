...
class RunnerBase:
    ...
    def __init__(self, cfg, task, model, datasets, job_id, output_dir_name=""):
        self.config = cfg
        self.job_id = job_id
    ...
    def setup_output_dir(self):
        ...
        # output_dir = Path(self.config.run_cfg.output_dir) / self.output_dir_name
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        # <CoCoOp에만 해당>
        # 아래 코드를 지우지 않으면 require gradient가 False인 key를 저장하지 않음
        # 그럼 missing key가 생기고 그들은 랜덤하게 초기화 됨
        # Pre-trained model을 load하지 않고 fine tuned model만 load하기 때문
        # Pre-trained model을 불러오고 그 위에 fine tuned model을 덮는 방법을 생각해보자! 
        ...
        _save_to = os.path.join(
            self.output_dir,
            "checkpoint_{}.pth".format("best" if is_best else cur_epoch-1),
        )
        if os.path.exists(_save_to):
            logging.info(f"Checkpoint {_save_to} already exists. Removing it.")
            os.remove(_save_to)
